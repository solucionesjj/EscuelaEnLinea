...
export class ConstanciaComponent implements OnInit {
  ...
  matriculationStudentInformationTmp:any = [];
  matriculationStudentInformation:any = [];
  today: number = Date.now();

  constructor(private userService: UserService, private reportService: ReportService, private schoolSevice: SchoolService, private alertService: AlertService, private courseService: CourseService, private radicadorValoracionesFinalesService: RadicadorValoracionesFinalesService) {
  ...
  selectYear() {
   ...
   getNextYearCourse(idStudent:number):string  {
    var course:string = '';
    this.matriculationStudentInformation.forEach(element => {
      if(element.studentId == idStudent) {
        course = element.nextCourse;
      }
    });
    return course;
  }
  ...
  async getRadicadorValoracionesFinalesData() {
    ...
    if (parseInt(this.selectedCourse) > 0) {
      ...
      if (result.length > 0) {
        ...
        this.students.forEach(student => {
          ...
          this.matriculationStudentInformationTmp.push({studentId: student.data[0].studentId})
        });
      } else {
    ...
    this.matriculationStudentInformationTmp.forEach(async info => {
      const result = await this.userService.getNextYearCourse(info.studentId);
      if(result) {
        this.matriculationStudentInformation.push({studentId:info.studentId,nextCourse:result.course}); 
      } else {
        this.matriculationStudentInformation.push({studentId:info.studentId,nextCourse:"(Sin matrícula)"}); 
      }
    });
  }
